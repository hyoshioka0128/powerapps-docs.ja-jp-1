...
ms.locfileid: "77145473"
---
# <a name="understand-data-form-layout-for-canvas-apps-in-power-apps"></a>Power Apps でのキャンバスアプリのデータフォームレイアウトについて
...
標準の 12 列よりもさらに細かい制御が必要な場合があります。 そのような場合は、 **[列へのスナップ]** をオフにして、カードを手動で配置します。 フォームは引き続き12列にスナップしますが、サイズ変更や位置変更を開始した後に Alt キーまたは Ctrl + Shift キーを押しながら、スナップポイントを上書きすることもできます。  詳細については、「[代替動作のキーボードショートカット](keyboard-shortcuts.md#alternate-behavior)」を参照してください。 

この例では、住所の 3 番目の行を形成する 4 つのコンポーネントが正確に同じ幅になっています。 しかし、これが最も適切なレイアウトではない場合があります。都市名は州の省略名よりも長く、国/地域のテキスト入力ボックスはラベルの長さのために短くなるからです。  この領域を最適化するには、右側のペインの [**列へのスナップ]** をオフにしてから、Alt キーまたは Ctrl + Shift キーを押したまま、これらのカードのサイズを設定して配置します。 For
...
| スナップのサイズ変更をオーバーライドできる |いいえ |はい。サイズ変更を開始した後、Alt キーまたは Ctrl キーを押しながら Shift キーを押します。 |
