...
| **レコード** | データ値のレコード。 この複合データ型には、このトピックに記載されている他のデータ型のインスタンスが含まれています。 詳細につい[ては、「テーブルの使用」を](../working-with-tables.md)参照してください。 | **{Company: "Northwind Traders",<br>スタッフ:35, <br>非営利団体: false}** |
...
| **テーブル** | レコードのテーブル。  すべてのレコードは、同じデータ型のフィールドに対して同じ名前を持つ必要があり、省略したフィールドは*空白*として扱われます。 この複合データ型には、このトピックに記載されている他のデータ型のインスタンスが含まれています。 詳細につい[ては、「テーブルの使用」を](../working-with-tables.md)参照してください。 | **テーブル ({FirstName: "Sidney",<br>LastName: "Higa"}, <br>{FirstName: "ナンシー",<br>LastName: "Anderson"})**
