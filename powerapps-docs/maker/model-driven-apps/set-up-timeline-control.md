...
10. 以下を、**追加オプション** コンテナで指定します。
    ...
    | Sort | タイムラインに表示されるレコードに基づいてソート順を選択します。 ソートは、活動用に選択したフィールドに基づいています。 投稿、メモ、または活動のフィールドが存在しない場合、並べ替えは **最終更新**フィールドに基づいて行われます。 <br><br>規定の並べ替え順序は、**降順** です。  | 降順 |
...
カスタマイザーとして、ビジネス要件に応じてユーザーに対して表示する活動を選択できます。
...
タイムライン セクションには、各活動のカードが表示されます (有効な活動に基づいて)。 各カードには、折りたたみモードと展開モードで特定のフィールドが表示されます。 たとえば、折り畳みモードと展開モードの**電子メールメール** 活動カードです。 
...
**詳細** セクションのフィールド **5** が折りたたみモードで表示されます。
...
5.  エンティティを選択し、 **フォーム** を選択します。 この例では、**タスク** エンティティ を選択します。

6.  リストから **タスク カード フォーム** レコードを選択します。 **タスクカード フォーム**が新しいブラウザ ウィンドウで開きます。
