...
||**管理プロパティ**|マネージド プロパティ コマンドには、**カスタマイズ可能**と**削除可能**の 2 つのプロパティがあります。 これらのプロパティを false に設定すると、フォームをソリューションに含ませ、管理ソリューションとしてそのソリューションをエクスポートし、別の環境にその管理ソリューションをインポートした後に、そのフォームはカスタマイズできなくなり、削除できません。 詳細: [管理プロパティ](/power-platform/alm/managed-properties-alm)| 
