...
単一テナント型サーバー間シナリオは通常、認証に Active Directory フェデレーション サービス (AD FS) を使用した複数の Common Data Service 環境を持つエンタープライズ組織に対して適用されます。 ただしアプリケーションが他の環境に配布されない場合は、環境によって適用することが可能です。  
 ...
 企業様は、単一 Azure Active Directory (Azure AD) テナント向けのすべての Common Data Service 環境に接続する Web アプリケーションまたはサービスを作成できます。
...
- Azure AD テナントおよび Common Data Service 環境の管理者特権。

<a name="bkmk_registration"></a>
## <a name="azure-application-registration"></a>Azure アプリケーション登録
Azure AD アプリケーション登録を作成するには、次の手順に従います。

1. https://admin.microsoft.comに移動してサインインするか、Common Data Service 環境の Web ページから左上隅にあるアプリケーション ランチャーを選択します。
...
5. **アプリケーションの登録** フォームにアプリの名前を指定し、**この組織のディレクトリ内のアカウントのみ** を選択して、**登録** を選択します。 リダイレクト URI は、このチュートリアルと付属のサンプル コードについては不要です。
    ...
    > ![申請フォームを登録します](media/S2S-app-registration-started.PNG)

6. **概要 ページ**で、**API のアクセス許可** を選択します 

    > [!div class="mx-imgBorder"]
    > ![アプリ登録許可](media/S2S-app-registration-completed.PNG)
...
9. **API アクセス許可の要求** フォームで、**委任されたアクセス許可** を選択し、**user_impersonation** を確認して、**アクセス許可の追加**を選択します 
    ...
    > [!div class="mx-imgBorder"]
    > ![API アクセス許可の設定](media/S2S-api-permission-started.PNG)

10. **API のアクセス許可**ページの下部の**同意する** で、**org-name に管理者の同意を与える**を選択し、確認メッセージが表示されたら **はい** を選択します 

    > [!div class="mx-imgBorder"]
    > ![API アクセス許可を付与しています](media/S2S-api-permission-completed.PNG)
...
ライセンスを付与されていない「アプリケーション ユーザー」を組織で作成するには、次の手順に従います。 このアプリケーション ユーザーは、アプリケーションを使用しているエンド ユーザーに代わって組織のデータにアクセスできます。

1. Common Data Service 環境 (https://*[org]*.crm.dynamics.com) に移動します。
2. **設定** > **セキュリティ** > **ユーザー**の順に移動します。
3. ビュー フィルターで **アプリケーション ユーザー** を選択します
4. **+ 新規**を選択します。
5. **アプリケーション ユーザー** フォームで、必要な情報を入力します。 

    1. ユーザー名情報は、Azure Active Directory に存在するユーザーに一致しないようにする必要があります。
    1. **アプリケーション ID** のフィールドに、以前に Azure AD に登録したアプリのアプリケーション ID を入力します。

    > [!div class="mx-imgBorder"]
    > ![新しいアプリのユーザー](media/S2S-new-appuser1.png)

6. 問題なくすべてを入力して **保存** を選択すると、**アプリケーション ID の URI** および **Azure AD オブジェクト ID** のフィールドに正しい値が自動入力されます。

    > [!div class="mx-imgBorder"]
    > ![新しいアプリのユーザー](media/S2S-new-appuser.PNG)

7. ユーザー フォームを終了する前に、アプリケーション ユーザーが目的の組織データにアクセスできるように **ロールの管理** を選択して、このアプリケーションのユーザーにセキュリティ ロールを割り当てます。

> [!IMPORTANT]
> S2S を使用して実際のアプリケーションを開発するときは、ソリューションに格納して、アプリケーションとともに配布できる、ユーザー定義のセキュリティ ロールを使用する必要があります。

## <a name="enable-or-disable-application-users"></a>アプリケーション ユーザーの有効化または無効化
アプリケーションユーザーは作成されると、自動的に有効になります。 既定のアプリケーション ユーザー の フォーム では、フォーム の フッター部分に**状態**が表示されます。 この **状態** フィールドを更新することはできません。  

アプリケーション ユーザーの状態が無効であり、有効にする必要があるイベントでは、次の手順に従って**状態**フィールドのアップデートを実行するようにアプリケーション ユーザー フォームをカスタマイズしてください。 これらの手順を使用して、使用されなくなったアプリケーション ユーザーを無効にすることもできます。

1. アプリケーション ユーザー フォーム フッターの**状態**フィールドを削除します。
    1. Common Data Service 環境 (https://*[org]*.crm.dynamics.com) に移動します。
    1. **設定** > **カスタマイズ** > **システムのカスタマイズ**の順に移動します。
    1. 左側のウィンドウで、**エンティティ** > **ユーザー** > **フォーム**を選択します。
    1. フォームの一覧で、**アプリケーション ユーザー**を選択します。
    1. [操作] バーで、**フッター**を選択します。
    1. [ステータス] グリッドをクリックし、アクション バーで [削除] を選択します。

    > [!div class="mx-imgBorder"]
    > ![アプリケーション ユーザー フォームの状態フィールドを削除する](media/remove-status-app-user.png "アプリケーション ユーザー フォームからの状態フィールドの削除")

1. アプリケーション ユーザー フォームの本文の新しいセクションへのフィールド**状態**を追加します。
    1. [操作] バーで、**本文**を選択します。
    1. **挿入**タブで、**セクション** > **1 列**を選択してください。
    1. **フィールド エクスプローラー**下で**状態**フィールドを見つけて、新しいセクション領域に**状態**フィールドをドラッグアンドドロップします。
 
    > [!div class="mx-imgBorder"]
    > ![アプリケーション ユーザー フォームに状態フィールドを追加する](media/add-status-app-user.png "アプリケーション ユーザー フォームに状態フィールドを追加する")

1. カスタマイズを保存して、公開します。

これで、[アプリケーション ユーザー] に移動して、[アプリケーション ユーザー] の有効化または無効化で求められる通り**状態**フィールドをアップデートします。
