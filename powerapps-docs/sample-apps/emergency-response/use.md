...
- **ベッドのキャパシティ**
    <br> 認可済ベッド、ICU ベッド、その他ベッドの収容能力データなどのベッド情報を収集します。

- **COVID-19 の統計**
    <br> COVID-19 の検査中である患者数と、検査が陽性と判定された患者数についての状態を収集します。

- **備品**
    <br> 人工呼吸器、NIPPV、PAPRなどの機器情報を追跡します。

- **スタッフ**
    <br> パートナー、割り当てられた、要求された、未割り当ての患者数と RN のステータス情報を収集します。
...
認可済ベッド、ICU（空気感染隔離室 /非空気感染隔離室）ベッド、急性期ケア（空気感染隔離室 /非空気感染隔離室）ベッド、選択した施設に、認可済ベッドの収容能力に見合った人員が配置しているかどうか、などのベッドに関連する情報を送信します。

変更を送信せずに **病院の緊急時対応アプリ** に戻る場合は、左上から **戻る** を選択します。 **送信** ボタンは入力した値を送信します。

データの送信後は、**他を追跡** ボタンを使用して、 **ベッドの収容能力** アプリに戻って別のレコードを作成することもできます。 **ホーム** を選択し、**病院の緊急対応アプリ** に戻ります。
...
| 現在使用中の ICU ベッド数（空気感染隔離室）               | 現在使用中の ICU ベッド数（空気感染隔離室）。                                      |
| 現在使用中の ICU ベッド数（非空気感染隔離室）           | 現在使用中の ICU ベッド数（非空気感染隔離室）。                                  |
| 現在使用中の急性期治療ベッド数（空気感染隔離室）        | 現在使用中の急性期治療ベッド数（空気感染隔離室）。                               |
| 現在使用中の急性期治療ベッド数（非空気感染隔離室）    | 現在使用中の急性期治療ベッド数（非空気感染隔離室）。                           |
| 認可された病床数の収容数を完全に活用できるだけの人員が配置されていますか？    | はい/いいえ　答えが「いいえ」の場合、該当するすべての理由を選択することができます： <br> - スタッフ <br> - 空き領域 <br> - PPE <br> - 機器 <br> - 患者数の少なさ |
| 許可ベッド数を超えて急増できますか?              | はい/いいえ　答えが「いいえ」の場合、該当するすべての理由を選択することができます： <br> - スタッフ <br> - 空き領域 <br> - PPE <br> - 機器 <br> - 患者数の少なさ  |
| 現在使用中の急増対応ベッドの数                         | 現在使用中の急増対応ベッド数。                                                |
...
## <a name="equipment"></a>機器

![機器](media/use/equipment.png)

 **備品** アプリを使用してその場所に特化した機器の詳細情報を送信します。人工呼吸器、NIPPV、PAPR などの使用中の機器の数を更新できます。
...
| PAPR ベルト        | 使用中の電動ファン付き空気清浄マスクベルトの数。                                                         |
| PAPR 充電器        | 使用中の電動ファン付き空気清浄マスク充電器の数。                                                         |
| PAPR フード        | 使用中の電動ファン付き空気清浄マスク フードの数。                                                         |
...
| **施設レベルの追跡** |                                                                                                   |
| この施設で現在不在となっている必須治療職員の比率                  | 現在不在となっている要治療者を、**施設全体**の総計に占める割合で表示しています。                    |
...
どの Emergency Response モバイル アプリのコンポーネントからでも、 **アプリのフィードバック** オプションを使用してフィードバックを共有することができます。 フィードバックを共有するには、左上からプロフィールを選択し、**フィードバックを送信する** ボタンを選択します。
